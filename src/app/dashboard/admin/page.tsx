...
import {
  ...
  BarChart2, LogOut, Search, Plus, CheckCircle, Clock, XCircle,
} from "lucide-react";
        ...
        <div className="mx-3 mb-4 bg-white/10 rounded-xl px-4 py-3">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-white/50 text-[11px] truncate">Platform Administra...</p>
            </div>
          </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-[1100px] mx-auto px-8 py-10">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-[38px] font-semibold text-[#1c2b1a] leading-tight" style={{ fontFamily: "Georgia, serif" }}>
              Platform overview{" "}
          ...
          <div className="grid grid-cols-5 gap-3 mb-8">
            {stats.map((s) => (
              ...
              >
                <p className={`text-[10px] font-semibold tracking-[0.15em] ${s.featured ? "text-white/50" : "text-[#9aaa8a]"}`}>
                  {s.label}
                </p>
                <p
                  className={`text-[28px] font-semibold leading-none ${s.featured ? "text-white" : "text-[#1c2b1a]"}`}
          ...
          <div className="grid grid-cols-[1fr_300px] gap-5 mb-6">
            ...
            <div className="bg-white border border-[#ede8df] rounded-2xl p-6">
              <div className="flex items-start justify-between mb-6">
                ...
                <select className="text-[12px] border border-[#e0dbd0] rounded-full px-4 py-1.5 text-[#4a5568] bg-[#faf9f6] focus:outline-none cursor-pointer">
                ...
                {salesData.map((d, i) => {
                  ...
                  return (
                    <div key={d.month} className="flex-1 flex flex-col items-center gap-2">
                      <span className="text-[11px] font-medium text-[#7a8a6a]">${d.value}</span>
                      <div
                        className={`w-full rounded-t-lg ${isCurrent ? "bg-[#1e3d18]" : "bg-[#c8e6c0]"}`}
                        style={{ height: `${barH}px` }}
                      />
                      <span className="text-[10px] font-semibold tracking-widest text-[#9aaa8a]">{d.month}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Top performers */}
            <div className="bg-white border border-[#ede8df] rounded-2xl p-6">
              <h2 className="text-[20px] font-semibold text-[#1c2b1a] mb-1" style={{ fontFamily: "Georgia, serif" }}>
                Top performers
              </h2>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 text-[13px] bg-[#faf9f6] border border-[#e0dbd0] rounded-full focus:outline-none focus:border-[#2d5a1b] transition-colors w-48"
                  />
                </div>
                <button className="flex items-center gap-2 px-5 py-2 bg-[#1e3d18] text-white rounded-full text-[13px] font-medium hover:bg-[#2d5a1b] transition-colors">
                  <Plus size={13} />
                  Add farmer
                </button>
              </div>
                ...
                {filteredFarmers.map((f, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-[#f8f6f2] last:border-0 hover:bg-[#faf9f6] transition-colors">
                      ...
                      <td className="py-4 pr-4 text-[13px] text-[#5a6a52]">{f.region}</td>
                      <td className="py-4 pr-4 text-[13px] text-[#5a6a52]">{f.products}</td>
                      <td className="py-4 pr-4">
                        <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold tracking-wide px-3 py-1 rounded-full ${s.color}`}>
                          {s.icon}
                          {f.status}
                        </span>
                      </td>
                      <td className="py-4 text-[13px] font-medium text-[#2d5a1b] hover:underline cursor-pointer">
                        View →
                      </td>
                    </tr>
                  );
